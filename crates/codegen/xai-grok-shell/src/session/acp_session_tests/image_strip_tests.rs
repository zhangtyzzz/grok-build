...
async fn rewind_cancels_terminal_image_strip_waiting_for_gate() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                !drain_gateway_debug(&mut gateway_rx).contains("removed from the conversation"),
                "cancelled terminal persistence must not emit a stale durable-removal note"
            );
        })
        .await;
}
...
#[tokio::test(flavor = "current_thread")]
async fn pending_strip_bound_preserves_waiting_and_new_url_entries() {
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
                ...
                .await;
            {
                ...
                assert!(
                    pending
                        .get(&applying_id)
                        .is_some_and(|strip| !strip.applying && !strip.urls.is_empty()),
                    "bound enforcement must retain terminal work waiting for the rewrite gate"
                );
                assert!(
                    pending
                        .get(&queued_id)
