...
async fn authenticated_401s_still_exhaust_after_three_retries_impl() {
    ...
    // The retry-budget terminal path must still notify the pager so it can
    // surface both the re-auth prompt and the failed-turn block.
    let (error_type, message) =
        terminal_failure(&updates).expect("an exhausted turn must report a terminal retryState");
    assert_eq!(
        error_type, "auth",
