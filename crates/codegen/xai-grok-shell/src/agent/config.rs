...
impl Config {
    ...
    pub fn feature(&self, feature: Feature) -> Resolved<bool> {
        if crate::privacy::is_hardened_build() && matches!(feature, Feature::Feedback) {
            return Resolved::new(false, ConfigSource::Default);
        }
...
pub fn find_model_with_key_by_id<'a>(
    ...
    let mut matches = models.iter().filter(|(key, entry)| {
        entry.info.has_model_id(model_id)
...
pub fn find_model_by_locator<'a>(
    ...
    if let Some(model_ref) = model_ref {
        ...
        return (entry.info.has_model_id(model) && endpoint_matches(&entry)).then_some(entry);
    }

    let mut matches = models
        .values()
        .filter(|entry| entry.info.has_model_id(model))
...
pub(crate) fn resolve_model_auth_facts_for_locator(
    model_ref: Option<&str>,
    model_id: &str,
    base_url: &str,
) -> ModelAuthFacts {
...
pub fn stamp_session_local_sampler_fields(
    ...
    cfg.bearer_resolver = (resolved.inherit_session_bearer
