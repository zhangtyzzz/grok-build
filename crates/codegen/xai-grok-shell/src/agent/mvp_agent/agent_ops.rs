...
impl MvpAgent {
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if prefers_oidc && !model.opts_out_of_ambient_credentials()
        ...
        if !has_session_key && credentials.auth_type == xai_chat_state::AuthType::ApiKey
            && !model.opts_out_of_ambient_credentials() && is_session_based_auth
        {
            tracing::info!(
        ...
        if should_warn_missing_session(MissingSessionCtx {
            has_session_key,
            has_own_credentials: model.opts_out_of_ambient_credentials(),
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let models = self.models_manager.models();
        let model_entry = sampling_config
            .model_ref
            .as_deref()
            .and_then(|model_ref| models.get(model_ref))
            .or_else(|| config::find_model_by_id(&models, &sampling_config.model));
        let subagent_rate_limit_max_attempts = self.resolved_subagent_rate_limit_max_attempts(
            model_entry.and_then(|entry| entry.info.subagent_rate_limit_max_attempts),
        );
        let model_max_retries = model_entry
