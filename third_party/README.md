...
grove-on-NFS userspace server (`nfsserve`). `smartstring` is vendored separately
to keep Rhai workflows buildable on the pinned stable Linux toolchain. Vendoring
gives a full audit surface, pins exact source, and avoids crates.io yanks. Local
